...
import { alterarStatusCliente } from "@/actions/clientes/alterar-status-cliente";
...
export function ClienteStatusButton({
  empresaId,
  clienteId,
  clienteNome,
  ativo,
  ...
  async function handleAlterarStatus() {
    ...
    try {
      ...
      setErro(
        "Não foi possível alterar o status do cliente. Tente novamente."
      );
    } finally {
      setCarregando(false);
    }
        ...
        >
          {vaiAtivar ? (
            ...
            <>
              ...
              <p className="mt-1 text-muted-foreground">
                O cliente deixará de ficar
                disponível para novas
                operações, mas continuará
                vinculado aos documentos já
                existentes.
              </p>
            </>
          )}
