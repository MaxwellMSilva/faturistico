...
import { alterarStatusNaturezaOperacao } from "@/actions/naturezas-operacao/alterar-status-natureza-operacao";
...
export function NaturezaStatusButton({
  empresaId,
  naturezaId,
  descricao,
  ativo,
  ...
  async function handleAlterarStatus() {
    ...
    try {
      ...
      const resultado =
        await alterarStatusNaturezaOperacao({
          empresaId,
          naturezaId,
          ativo: vaiAtivar,
        });
      ...
      setErro(
        "Não foi possível alterar o status da natureza de operação. Tente novamente."
      );
    } finally {
      setCarregando(false);
    }
    ...
    >
      ...
      <AlertDialogContent>
        <AlertDialogHeader>
          ...
          <AlertDialogTitle>
            {vaiAtivar
              ? "Ativar natureza de operação"
              : "Inativar natureza de operação"}
          </AlertDialogTitle>
          ...
          {vaiAtivar ? (
            <>
              <p className="font-medium text-emerald-700 dark:text-emerald-400">
                A natureza voltará a ficar
                disponível.
              </p>

              <p className="mt-1 text-muted-foreground">
                Ela poderá ser selecionada
                novamente em novas notas
                fiscais.
              </p>
            </>
          ) : (
            <>
              ...
              <p className="mt-1 text-muted-foreground">
                A natureza deixará de ficar
                disponível para novas notas,
                mas continuará vinculada aos
